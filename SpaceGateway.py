...
"""
 ...
 THIS VERSION IS MEANT TO SUPPORT AND NOT REPLACE THE ORIGINAL VERSION AUTHORED BY AFRICASTALIKNG.
 CLASS IS AN EXTENSION FOR PYTHON 3+ AND ABOVE.
...
class SpaceTalking:
    ...
    def __init__(self,username,apiKey_):
    ...
    def sendMessage(self, to_, message_, from_ = None, bulkSMSMode_ = 1, enqueue_ = 0, keyword_ = None, linkId_ = None, retryDurationInHours_ = None):
        if len(to_) == 0 or len(message_) == 0:
            raise SpaceException("Please provide both to_ and message_ parameters")
        parameters = {'username':self.username,'to': to_, 'message':message_,'bulkMSMMode':bulkSMSMode_}
    ...
    def createSubscription(self, phoneNumber_, shortCode_, keyword_):
        if len(phoneNumber_) == 0 or len(shortCode_) == 0 or len(keyword_) == 0:
            raise SpaceException("Please supply phone number, short code and keyword")
        url        = "%s/create" %(self.SubscriptionURLString)
        parameters = {'username':self.username,'phonenumber':phoneNumber_,'shortCode':shortCode_,'keyword':keyword_}
    ...
    def fetchPremiumSubscriptions(self,shortCode_, keyword_, lastReceivedId_ = 0):
        if len(shortCode_) == 0 or len(keyword_) == 0:
            raise SpaceException("Please supply the short code and keyword")
    ...
    def call(self, from_, to_):
        parameters = {'username':self.username,'from':from_,'to':to_}
        ...
        raise SpaceException(decoded['errorMessage'])

    def getNumQueuedCalls(self, phoneNumber_, queueName_ = None):
        parameters = {'username':self.username,'phoneNumber':phoneNumber_,'queueName':queueName_}
    ...
    def uploadMediaFile(self, urlString_):
        parameters = {'username':self.username,'url':urlString_}
    ...
    def sendAirtime(self, recipients_):
        parameters = {'username':self.username,'recipients':recipients_}
    ...
    ##############################################################################
    ###Request Handle Methos
    ##############################################################################
    def sendRequest(self,urlString,data=None):
        try:
            if data is not None:
                request = requests.post(urlString,data,headers=self.headers)
            else:
                raise SpaceException('Parameters missing')
            response = request.text
            self.responseCode = request.status_code
            if self.Debug:
                print(response)
            return response
        except Exception as error:
            raise SpaceException(str(error))
